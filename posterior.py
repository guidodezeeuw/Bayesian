...
def best_guesses(matrix_generalized,model_class,params):
    ...
    for i in range(iterations):
        #print('N=',N)
        boundaries = np.zeros(model_class+1)                          #empty list in which boundaries will be stored --> [start_depth,boundary_1,boundary_2,...,end_depth]
        # P_ST_J=np.zeros((model_class,9))                            #matrix in which results following from eq. 2 (Wang et al., 2013) are stored
        # P_eps_n=np.zeros(model_class)                               #matrix in which results following from eq. 1 (Wang et al., 2013) are stored
...
def calculator(matrix_generalized,params):
    """
    main function solving for the different parts
    """
    ...
    k = np.arange(N_min,N_max+1)

    """1a. if a probability = 0, this will cause problems --> change all 0's to a small number"""
    ...
    K_N_L = np.zeros((1,N_max))

    """2. minimize objective function (=-ln(likelihood function)) per model class"""
    """2a. Model class 1: having 1 layer, so only 1 solution"""
    N1 = objective_function([thickness_tot],matrix_generalized)
    ...
    """3. calculate most probable model class"""       
    #likelihood = np.exp(-all_optimized[:,0])
    print(k, std_Fr, std_Qt)
    likelihood = -all_optimized[:,0]
    prior_distribution = np.log((1/std_Fr * 1/std_Qt)**k)
    print(likelihood,prior_distribution)
    
    #calculate conditional probability (Eq. 12, Wang et al. (2013))
    cond_prob = likelihood+prior_distribution
    cond_prob = np.exp(cond_prob)
    cond_prob = np.log(cond_prob/(thickness_tot**(k-1)))
    print(cond_prob)
    #maximize posterior function and select layer and thicknesses
    max_index = np.argmax(cond_prob)
    final_class = all_optimized[max_index,:max_index+2]
    
    """4. get soil types for that model class"""
